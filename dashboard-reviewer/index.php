...
if (isset($_SESSION['id_user']) && isset($_SESSION['level']) && $_SESSION['level'] == 'reviewer' ) {
		...
		$sql_user->execute();
		$sql_satker = $link->prepare("SELECT * FROM satker WHERE id_user = '$id_user' ");
		$sql_satker->execute();
		$user = 	$sql_user->fetch(PDO::FETCH_OBJ);
		$satker = 	$sql_satker->fetch(PDO::FETCH_OBJ);
